...
try:
    from tensorgraph.models_zoo.aibraintumormodel.nn.model \
        import model_C5XS as model_module
except:
    warnings.warn("WARNING: Unable to locate model_C5XS in TensorGraph package. Attempting relative path import")
    model_path = "../../../tensorgraph/models_zoo/"
    sys.path.append(model_path)
    from aibraintumormodel.nn.model import model_C5XS as model_module
...
def test_model():
    with tf.Graph().as_default():
        ...
        nclass0, nclass1 = (4,8)
        nseg = 2
        nclass_mat = np.zeros((nclass1, nclass0), dtype=np.int32)
